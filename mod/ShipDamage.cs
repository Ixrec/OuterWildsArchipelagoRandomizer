...
internal class ShipDamage
{
    ...
    private static void DamageShip()
    {
        if (shipDamageController != null)
        {
            ...
            if (components.Count > 0)
            {
                ...
                if (components.Count > 0)
                {
                    ...
                    if (component1.name.Contains("ThrusterBankComponent"))
                    {
                        ...
                        {
                            Randomizer.OWMLModConsole.WriteLine($"this trap already disabled one thruster bank, so excluding the other thruster bank");
            ...
            Randomizer.OWMLModConsole.WriteLine($"Ship Damage Trap chose to damage hulls: [{string.Join(", ", damagedHullNames)}] and components: [{string.Join(", ", damagedComponentNames)}]");

            if (!PlayerState.IsInsideShip())
            {
                Randomizer.OWMLModConsole.WriteLine($"generating notification about ship damage because player is outside the ship");
